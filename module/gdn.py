...
class SlowGatedDeltaNet(nn.Module):
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.dt_bias = nn.Parameter(torch.ones(self.num_v_heads) * 0.5)
    ...
    def forward(
        ...
        **kwargs: Unpack[Dict]
        ...
        g = torch.relu(self.a_proj(hidden_states).float() + self.dt_bias)
        g = 2 / (torch.exp(g) + torch.exp(-g))
        g = g.log()
...
class SlowGatedDeltaNetBlock(GatedDeltaNetBlock):
    def __init__(self, config, layer_idx):
        super().__init__(config, layer_idx)
        self.attn = SlowGatedDeltaNet(
